...
def neuron2_action(idx: int) -> str:
    weights = ", ".join(f"bit<16> w{i}" for i in range(32))
    acc_lines = "\n".join(
        f"        acc = acc + ((int<32>)(int<16>)w{i} * ((int<32>)hdr.nn.r{i} >> 9));"
        for i in range(32)
    )
    return f"""
    action set_bias2_{idx}(int<32> b) {{
        bias2_{idx} = (int<32>)b;
    }}

    table neuron2_{idx}_bias {{
        actions = {{ set_bias2_{idx}; }}
        default_action = set_bias2_{idx}(0);
    }}

    action compute_neuron2_{idx}({weights}) {{
        int<32> acc = 0;
{acc_lines}
        acc = acc + (bias2_{idx} * (int<32>)512);
        if (acc > 0) {{
            hdr.nn.r{idx} = acc;
        }} else {{
            hdr.nn.r{idx} = 0;
    ...
    table neuron2_{idx}_weights {{
        actions = {{ compute_neuron2_{idx}; NoAction; }}
        default_action = NoAction();
    }}
"""


def neuron2_block(n0: int, n1: int) -> tuple[str, str, str, str]:
    if n0 > n1:
        return "", "", "", ""
    bias_decls = "\n".join(f"    int<32> bias2_{i};" for i in range(n0, n1 + 1))
    actions = "\n".join(neuron2_action(i) for i in range(n0, n1 + 1))
    apply_bias = " ".join(f"neuron2_{i}_bias.apply();" for i in range(n0, n1 + 1))
    apply_w = " ".join(f"neuron2_{i}_weights.apply();" for i in range(n0, n1 + 1))
    return bias_decls, actions, apply_bias, apply_w
...
def stage1_program() -> str:
    ...
    bias2, neuron2_actions, apply_bias2, apply_w2 = neuron2_block(0, 3)
...
def stage_mid_program(name: str, n0: int, n1: int, n2_0: int, n2_1: int) -> str:
    ...
    bias2, neuron2_actions, apply_bias2, apply_w2 = neuron2_block(n2_0, n2_1)
...
def stage4_program() -> str:
    ...
    bias2, neuron2_actions, apply_bias2, apply_w2 = neuron2_block(12, 15)
...
def main() -> None:
    ...
    (OUT_DIR / "ids_nn_s2.p4").write_text(stage_mid_program("s2", 8, 15, 4, 7))
    (OUT_DIR / "ids_nn_s3.p4").write_text(stage_mid_program("s3", 16, 23, 8, 11))
