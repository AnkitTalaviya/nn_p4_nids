...
control MyIngress(inout headers hdr,
    ...
    apply {
        if (hdr.ipv4.isValid() && hdr.nn.isValid() && hdr.nn.magic == NN_MAGIC) {
            neuron23_bias.apply(); neuron24_bias.apply(); neuron25_bias.apply(); neuron26_bias.apply(); neuron27_bias.apply();
            active_local_neuron_count_reg.read(active_local_neuron_count, 0);
            if (active_local_neuron_count > 0) { neuron23_weights.apply(); } else { hdr.nn.r23 = 0; }
            if (active_local_neuron_count > 1) { neuron24_weights.apply(); } else { hdr.nn.r24 = 0; }
            if (active_local_neuron_count > 2) { neuron25_weights.apply(); } else { hdr.nn.r25 = 0; }
            if (active_local_neuron_count > 3) { neuron26_weights.apply(); } else { hdr.nn.r26 = 0; }
            if (active_local_neuron_count > 4) { neuron27_weights.apply(); } else { hdr.nn.r27 = 0; }
            ipv4_lpm.apply();
