...
def infer_offline_quant_logits(x_q: np.ndarray, model: Dict[str, object]) -> np.ndarray:
    w1 = np.array(model["layer_0"]["weights"], dtype=np.int32)  # input_dim x 4
...
def pad_to_9_features(features_q: np.ndarray) -> np.ndarray:
    arr = np.asarray(features_q, dtype=np.int16).reshape(-1)
    if arr.size >= 9:
        return arr[:9]
    out = np.zeros(9, dtype=np.int16)
    out[: arr.size] = arr
    return out


def build_inject_packet(features_q: np.ndarray, ttl: int = 64) -> IP:
    # P4 feature header uses 9x16-bit words; pad with zeros when model input < 9.
    feat9 = pad_to_9_features(features_q)
    words = [int(v) & 0xFFFF for v in feat9.tolist()]
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(description="5-4-1 BMv2 accuracy/confusion-matrix test")
    ap.add_argument("--dataset", type=Path, default=repo_root / "data/newDataSet.csv")
    ap.add_argument("--model-json", type=Path, default=repo_root / "5-4-1/offline/python/output/ptq_model_5_4_1.json")
...
def main() -> int:
    ...
    print("=" * 80)
    print("5-4-1 REAL BMv2 ACCURACY TEST")
