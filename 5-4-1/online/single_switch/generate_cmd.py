#!/usr/bin/env python3
"""Generate BMv2 runtime commands for 5-4-1."""
...
def load_model(model_json: Path):
    ...
    input_dim = int(arch.get("input", 0))
    if input_dim < 1 or input_dim > 9 or arch.get("hidden") != [4] or arch.get("output") != 1:
        raise ValueError(f"Expected 5-4-1 architecture, got {arch}")

    w1 = np.array(data["layer_0"]["weights"], dtype=np.int16)
    b1 = np.array(data["layer_0"]["biases"], dtype=np.int32)   # 4
    w2 = np.array(data["layer_1"]["weights"], dtype=np.int16).reshape(-1)  # 4
    if w1.shape != (input_dim, 4):
        raise ValueError(f"Expected layer_0 weights shape ({input_dim}, 4), got {w1.shape}")
    if b1.shape != (4,):
        raise ValueError(f"Expected layer_0 bias shape (4,), got {b1.shape}")
    if w2.shape != (4,):
        raise ValueError(f"Expected layer_1 weights shape (4,), got {w2.shape}")

    b2 = int(data["layer_1"]["biases"][0])
    threshold = int(data.get("decision", {}).get("threshold", 0))
    feature_mask = int(data.get("feature_mask", (1 << input_dim) - 1))
    feature_map = list(data.get("feature_map", list(range(input_dim))))
    if len(feature_map) < 9:
        feature_map = feature_map + [0] * (9 - len(feature_map))
    return w1, b1, w2, b2, threshold, feature_mask, feature_map[:9], input_dim
...
def build_commands(
    ...
    # Hidden layer: 32 neurons in P4, first 4 active from 5-4-1 model.
    for i in range(32):
        if i < 4:
            # w1 is input_dim x 4; map to 9 slots expected by P4.
            wi = [0] * 9
            for j in range(min(input_dim, 9)):
                wi[j] = int(w1[j, i])
            bi = int(b1[i])
        else:
            wi = [0] * 9
...
def parse_args() -> argparse.Namespace:
    ...
    default_model = here.parents[2] / "offline/python/output/ptq_model_5_4_1.json"
    default_out = script_dir / "commands.txt"
    ap = argparse.ArgumentParser(description="Generate commands.txt for 5-4-1 online deployment")
...
def main() -> None:
    args = parse_args()
    w1, b1, w2, b2, threshold, feature_mask, feature_map, input_dim = load_model(args.model_json)
    cmds = build_commands(w1, b1, w2, b2, threshold, feature_mask, feature_map, input_dim)
