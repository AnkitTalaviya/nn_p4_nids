...
# 5-4-1 end-to-end BMv2 deployment + accuracy/confusion-matrix test.
...
MODEL_JSON="${MODEL_JSON:-${REPO_ROOT}/5-4-1/offline/python/output/ptq_model_5_4_1.json}"
DATASET="${DATASET:-${REPO_ROOT}/data/newDataSet.csv}"
THRIFT_PORT="${THRIFT_PORT:-9090}"
SAMPLE_COUNT="${SAMPLE_COUNT:-500}"              # 0 => full dataset
...
ACC_LOG="${OUT_DIR}/final_switch_test_5_4_1.log"
ACC_CSV="${OUT_DIR}/final_switch_results_5_4_1.csv"
P4_TMP_LOG="$(mktemp "/tmp/5_4_1_p4c_XXXX.log")"
SW_TMP_LOG="$(mktemp "/tmp/5_4_1_bmv2_XXXX.log")"
...
echo "--- 0) GENERATE COMMANDS (5-4-1 -> BMv2 tables) ---"
...
echo "--- 5) RUN 5-4-1 ACCURACY + CONFUSION MATRIX TEST ---"
