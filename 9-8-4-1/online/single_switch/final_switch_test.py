#!/usr/bin/env python3
"""BMv2 accuracy test for 9-8-4-1."""
...
def _normalize_w1(w1_raw: np.ndarray, h1: int) -> np.ndarray:
    if w1_raw.shape == (9, h1):
        return w1_raw
    if w1_raw.shape == (h1, 9):
        return w1_raw.T
    raise ValueError(f"Unexpected layer_0 weight shape {w1_raw.shape}; expected 9x{h1} or {h1}x9")


def _normalize_w2(w2_raw: np.ndarray, h1: int, h2: int) -> np.ndarray:
    if w2_raw.shape == (h1, h2):
        return w2_raw
    if w2_raw.shape == (h2, h1):
        return w2_raw.T
    raise ValueError(f"Unexpected layer_1 weight shape {w2_raw.shape}; expected {h1}x{h2} or {h2}x{h1}")
...
def _infer_offline_quant_logits(x_q: np.ndarray, model: Dict[str, object], scale: int) -> np.ndarray:
    arch = model.get("architecture", {})
    hidden = arch.get("hidden", [])
    if not isinstance(hidden, list) or len(hidden) != 2:
        raise ValueError(f"Expected two hidden layers in architecture, got: {arch}")
    h1 = int(hidden[0])
    h2 = int(hidden[1])

    w1 = np.array(model["layer_0"]["weights"], dtype=np.int32)
    b1 = np.array(model["layer_0"]["biases"], dtype=np.int32).reshape(-1)

    w2 = np.array(model["layer_1"]["weights"], dtype=np.int32)
    b2 = np.array(model["layer_1"]["biases"], dtype=np.int32).reshape(-1)

    w3 = np.array(model["layer_2"]["weights"], dtype=np.int32).reshape(-1)
    b3 = int(np.array(model["layer_2"]["biases"], dtype=np.int32).reshape(-1)[0])

    w1 = _normalize_w1(w1, h1)
    w2 = _normalize_w2(w2, h1, h2)

    if len(b1) != h1 or len(b2) != h2 or len(w3) != h2:
        raise ValueError("Model layer dimensions do not match architecture")

    x32 = x_q.astype(np.int32)
    z1 = x32 @ w1.astype(np.int32) + (b1.astype(np.int32) * scale)
    a1 = np.maximum(0, z1)
    a1s = a1 >> SHIFT

    z2 = a1s.astype(np.int32) @ w2.astype(np.int32) + (b2.astype(np.int32) * scale)
    a2 = np.maximum(0, z2)
    a2s = a2 >> SHIFT

    z3 = a2s.astype(np.int32) @ w3.astype(np.int32) + b3
    return z3.astype(np.int32)


def _build_packet(features_q: np.ndarray, ttl: int = 64) -> IP:
    ...
    return IP(src="10.0.0.1", dst="10.0.0.2", ttl=int(ttl), proto=17) / UDP(
        sport=FEATURE_SPORT_BASE + random.randint(0, 10000),
        dport=FEATURE_PORT,
    ) / Raw(load=payload)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="9-8-4-1 BMv2 accuracy/confusion-matrix test")
    ...
    p.add_argument("--iface", type=str, default="veth0_host")
    ...
    p.add_argument("--live-table-max-rows", type=int, default=200)
...
def main() -> int:
    ...
    x_q, y_true, features, scale = _prepare_quantized_features(df, model)
    ...
    rows = len(df)
    print("=" * 88)
    print("9-8-4-1 REAL BMv2 ACCURACY TEST")
