#!/usr/bin/env python3
"""Generate BMv2 runtime commands for 9-8-1."""
...
def load_model(model_json: Path):
    ...
    if arch.get("input") != 9 or arch.get("hidden") != [8] or arch.get("output") != 1:
        raise ValueError(f"Expected 9-8-1 architecture, got {arch}")

    w1 = np.array(data["layer_0"]["weights"], dtype=np.int16)  # 9 x 8
    b1 = np.array(data["layer_0"]["biases"], dtype=np.int32)   # 8
    w2 = np.array(data["layer_1"]["weights"], dtype=np.int16).reshape(-1)  # 8
...
def build_commands(
    ...
    # Hidden layer: 32 neurons in P4, first 8 active from 9-8-1 model.
    for i in range(32):
        if i < 8:
            # w1 is 9 x 8; extract weights for neuron i in feature order.
    ...
    # Output layer: pad 8 -> 32.
    w2_32 = np.zeros(32, dtype=np.int16)
    w2_32[:8] = w2.astype(np.int16)
    cmds.append(
...
def parse_args() -> argparse.Namespace:
    ...
    default_model = here.parents[2] / "offline/python/output/ptq_model_9_8_1.json"
    default_out = script_dir / "commands.txt"
    ap = argparse.ArgumentParser(description="Generate commands.txt for 9-8-1 online deployment")
