...
def infer_offline_quant_logits(x_q: np.ndarray, model: Dict[str, object]) -> np.ndarray:
    w1 = np.array(model["layer_0"]["weights"], dtype=np.int32)  # 9x8
    b1 = np.array(model["layer_0"]["biases"], dtype=np.int32)   # 8
    w2 = np.array(model["layer_1"]["weights"], dtype=np.int32).reshape(-1)  # 8
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(description="9-8-1 BMv2 accuracy/confusion-matrix test")
    ap.add_argument("--dataset", type=Path, default=repo_root / "data/UNSW_NB15_testing-set.csv")
    ap.add_argument("--model-json", type=Path, default=repo_root / "9-8-1/offline/python/output/ptq_model_9_8_1.json")
...
def main() -> int:
    ...
    print("=" * 80)
    print("9-8-1 REAL BMv2 ACCURACY TEST")
