#!/usr/bin/env python3
"""
Offline training + evaluation pipeline for a 9-8-1 IDS model.
...
def infer_float_logits(clf: MLPClassifier, x_scaled: np.ndarray) -> np.ndarray:
    ...
    w2 = np.asarray(clf.coefs_[1], dtype=np.float64).reshape(-1)  # 8
...
def run_pipeline(
    ...
    metrics_path = output_dir / "offline_metrics_9_8_1.json"
    ...
    pred_path = output_dir / "offline_test_predictions_9_8_1.csv"
    pred_df.to_csv(pred_path, index=False)

    # Export model artifact for later control-plane conversion.
    model_obj = {
        "model_name": "unsw_nb15_9_8_1_offline",
        "architecture": {"input": 9, "hidden": [8], "output": 1},
        ...
        "layer_0": {
            "weights": w1_q.astype(np.int16).tolist(),  # shape: 9 x 8
            "biases": b1_q.astype(np.int32).tolist(),   # shape: 8
        },
        "layer_1": {
            "weights": w2_q.astype(np.int16).reshape(-1).tolist(),  # shape: 8
            "biases": [int(b2_q)],
        },
        "decision": {
    ...
    model_path = output_dir / "ptq_model_9_8_1.json"
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Train/evaluate offline UNSW-NB15 9-8-1 model")
...
def main() -> None:
    ...
    m = result["metrics"]
    print("=== 9-8-1 Offline Training Complete ===")
